...
const FinancePage = () => {
  ...
  const financeItems = items.filter(item => item.category === "finance");
  const financeCategory = categories.find(c => c.id === "finance");
  const totalValue = financeCategory?.value || 0;
  const itemCount = financeItems.length;

  // Initialize filtered items
  useEffect(() => {
    setFilteredItems(financeItems);
  }, [financeItems]);
  ...
  return (
    <main className="lg:mr-64 pt-6 pb-12 px-4 md:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <SectionStats items={financeItems} category="finance" />
